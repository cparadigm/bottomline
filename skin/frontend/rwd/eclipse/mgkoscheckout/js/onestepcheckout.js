...
MGKOSC.prototype = {
    ...
    _addEventListeners: function () {
        $('login-form') && $('login-form').observe('submit', function (e) {
            ...
            new Ajax.Request($('login-form').action, {
                ...
                onSuccess: function (transport) {
                    MGKOSC.Messenger.clear('login-form');
                    var response = transport.responseText.evalJSON();
                    if (response.error) {
                        MGKOSC.Messenger.add(response.error, 'login-form', 'error')
                    }
        ...
        $('forgot-password-form') && $('forgot-password-form').observe('submit', function (e) {
            ...
            new Ajax.Request($('forgot-password-form').action, {
                ...
                onSuccess: function (transport) {
                    MGKOSC.Messenger.clear('forgot-password-form');
                    $('forgot-please-wait').hide();
                    $('btn-forgot').removeAttribute('disabled');
                  
                    var response = transport.responseText.evalJSON();
                    if (response.error) {
                        MGKOSC.Messenger.add(response.error, 'forgot-password-form', 'error')
                    } else if (response.message) {
                        MGKOSC.Messenger.add(response.message, 'forgot-password-form', 'success')
    ...
    save: function () {
        ...
        MGKOSC.Messenger.clear('checkout-review-submit');
        $$('#checkout-review-submit .checkout-agreements input[type="checkbox"]').each(function (el) {
            if (!el.checked) {
                MGKOSC.Messenger.add(this.acceptAgreementText, 'checkout-review-submit', 'error');
                isValid = false;
                throw $break
            }
        }.bind(this));
...
BillingAddress.prototype = {
    ...
    newAddress: function (isNew) {
        if (isNew) {
            ...
            Element.show('mgkosc-billingform')
        } else {
            Element.hide('mgkosc-billingform')
        }
    },
...
ShippingAddress.prototype = {
    ...
    newAddress: function (isNew) {
        if (isNew) {
            this.resetSelectedAddress();
            Element.show('mgkosc-shppingform')
        } else {
            Element.hide('mgkosc-shppingform')
        }
    },
    resetSelectedAddress: function () {
...
ShippingMethod.prototype = {
    ...
    validate: function () {
        MGKOSC.Messenger.clear('checkout-shipping-method-load');
        var methods = document.getElementsByName('shipping_method');
        if (methods.length == 0) {
            MGKOSC.Messenger.add(Translator.translate('Your order cannot be completed at this time as there is no shipping methods available for it. Please make neccessary changes in your shipping address.'), 'checkout-shipping-method-load', 'error');
            return false
        }
        ...
        MGKOSC.Messenger.add(Translator.translate('Please specify shipping method.'), 'checkout-shipping-method-load', 'error');
        return false
    }
};
...
Payment.prototype = {
    ...
    validate: function () {
        MGKOSC.Messenger.clear('checkout-payment-method-load');
        ...
        if (methods.length == 0) {
            MGKOSC.Messenger.add(Translator.translate('Your order cannot be completed at this time as there is no payment methods available for it.'), 'checkout-payment-method-load', 'error');
            return false
        }
        for (var i = 0; i < methods.length; i++) {
            if (methods[i].checked) {
        ...
        MGKOSC.Messenger.add(Translator.translate('Please specify payment method.'), 'checkout-payment-method-load', 'error');
        return false
    },
...
MGKOSC.Window = Class.create();
MGKOSC.Window.prototype = {
...
$(document).on("click",'#checkout-payment-method-load',function(){
    checkout.update({
	  'review': 1
    })
});