...
class BuyxgetyFixdisc extends Buyxgety
{
    ...
    protected function _calculate($rule, $item)
    {
        ...
        $maxQty = $this->ruleQuantity($realQty, $rule);
        ...
        foreach ($allItems as $i=>$allItem) {
            ...
            if (in_array($item->getAmrulesId(), $itemsId) && $allItem->getAmrulesId() === $item->getAmrulesId()) {
                ...
                $discountData->setOriginalAmount($qty * $quoteAmount);
                $discountData->setBaseOriginalAmount($qty * $rule->getDiscountAmount());
            }
        }
        return $discountData;
    }
