...
class AfternFixdisc extends AbstractRule
{
    ...
    protected function _calculate($rule, $item)
    {
        ...
        foreach ($allItems as $i => $allItem) {
            if (in_array($item->getAmrulesId(), $itemsId) && $allItem->getAmrulesId()===$item->getAmrulesId()) {
                ...
                $discountData->setAmount($itemQty * $rule->getDiscountAmount());
                $discountData->setBaseAmount($itemQty * $quoteAmount);
                $discountData->setOriginalAmount($itemQty * $rule->getDiscountAmount());
