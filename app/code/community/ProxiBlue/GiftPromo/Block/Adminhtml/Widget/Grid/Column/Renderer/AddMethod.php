    ...
    extends Mage_Adminhtml_Block_Widget_Grid_Column_Renderer_Select
{
    ...
    public function render(Varien_Object $row)
    {
        ...
        $html = '<select id="add-method-'.$row->getId().'" onChange="setRemoveable('.$row->getId().',this.options[this.selectedIndex].value)" class="add-method" name="' . $this->escapeHtml($name) . '" ' . $this->getColumn()->getValidateClass() . '>';
        $value = $row->getData($this->getColumn()->getIndex());
        $options = $this->getColumn()->getOptions();
        foreach ($options as $val => $label){
            if($row->getTypeId() == Mage_Catalog_Model_Product_Type_Configurable::TYPE_CODE && $label == 'Direct to cart') {
                continue;
            }
