...
CREATE TABLE  IF NOT EXISTS `{$couponUsageTable}` (
  ...
  CONSTRAINT `FK_GIFTPROMO_PROMO_COUPON_USAGE_CUSTOMER_ID_CUSTOMER_ENTITY_ID` FOREIGN KEY (`customer_id`) REFERENCES `{$installer->getTable('customer_entity')}` (`entity_id`) ON DELETE CASCADE ON UPDATE CASCADE
...
$installer->addAttribute('order', 'applied_gift_rule_ids', array(
...
CREATE TABLE  IF NOT EXISTS `{$installer->getTable('giftpromo/promo_rule_customer')}` (
  ...
  CONSTRAINT `FK_GIFTPROMO_CUSTOMER_CUSTOMER_ID_CUSTOMER_ENTITY_ENTITY_ID` FOREIGN KEY (`customer_id`) REFERENCES `{$installer->getTable('customer_entity')}` (`entity_id`) ON DELETE CASCADE ON UPDATE CASCADE,
