...
class ProxiBlue_GiftPromo_Model_Promo_Rule_Condition_Twitter_Conditions extends Mage_Rule_Model_Condition_Combine {
    ...
    public function asHtml() {
        if (self::getTwitterHandle()){
            $html = $this->getTypeElement()->getHtml() .
                    Mage::helper('rule')->__('If %s these conditions are %s:', $this->getAggregatorElement()->getHtml(), $this->getValueElement()->getHtml());
            if ($this->getId() != '1') {
    ...
    public function getNewChildSelectOptions() {
        ...
        $conditions = array_merge_recursive($conditions, array(
            array('value' => 'giftpromo/promo_rule_condition_twitter_conditions_follow', 'label' => Mage::helper('giftpromo')->__('Follows')),
        ));
        return $conditions;
    }
