...
class ProxiBlue_GiftPromo_Model_Promo_Rule_Condition_Customer_Action_Register extends Mage_Rule_Model_Condition_Abstract {
    ...
    public function asHtml() {
        $html = $this->getTypeElement()->getHtml() .
                Mage::helper('giftpromo')->__("If the customer registers during checkout (Note: Customer Group in rule information must be set to 'Not Logged In' group)");
        if ($this->getId() != '1') {
    ...
    public function validate(Varien_Object $object) {
        ...
        $result = false;
        if(is_object($object->getQuote()) && $object->getQuote()->getCheckoutMethod() == Mage_Checkout_Model_Type_Onepage::METHOD_REGISTER){
                $result = true;
