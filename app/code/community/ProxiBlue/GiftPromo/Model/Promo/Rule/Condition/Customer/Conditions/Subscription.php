 ...
 */
class ProxiBlue_GiftPromo_Model_Promo_Rule_Condition_Customer_Conditions_Subscription extends Mage_Rule_Model_Condition_Abstract {
    ...
    public function asHtml() {
        $html = $this->getTypeElement()->getHtml() .
                Mage::helper('giftpromo')->__("If the customer newsletter subscription status is %s", $this->getOperatorElement()->getHtml());
        if ($this->getId() != '1') {
            ...
            if ($newsletter->getId()) {
                switch ($op) {
                    ...
                    case 'SUBSCRIBED_UNCONFIRMED':
                        if ($newsletter->getSubscriberStatus() == Mage_Newsletter_Model_Subscriber::STATUS_UNCONFIRMED OR
                                $newsletter->getSubscriberStatus() == Mage_Newsletter_Model_Subscriber::STATUS_SUBSCRIBED) {
                            $result = true;
                        }
                        break;
                    case 'UNSUBSCRIBED':
                        if ($newsletter->getSubscriberStatus() == Mage_Newsletter_Model_Subscriber::STATUS_UNSUBSCRIBED) {
                            $result = true;
                        }
                        break;
                }
