...
class ProxiBlue_GiftPromo_Model_Promo_Rule_Condition_Twitter_Api extends Mage_Rule_Model_Condition_Abstract {
    ...
    private function fetchCachedEndpointResult($additional_cache_key) {
        ...
        if (is_array($cache)) {
            try {
                $time = time();
                if (mage::getStoreConfig('giftpromo/twitter/use_cache') && array_key_exists($this->_endpoint . $additional_cache_key, $cache) && $time - $cache[$this->_endpoint . $additional_cache_key]['timestamp'] < mage::getStoreConfig('giftpromo/twitter/cache_period')) {
