 ...
 */
class ProxiBlue_GiftPromo_Model_Promo_Rule_Condition_Customer_Conditions_Numsales extends Mage_Rule_Model_Condition_Abstract {

    protected $_inputType = 'text';

    public function __construct() {
        Mage_Rule_Model_Condition_Abstract::__construct();
        $this->setType('giftpromo/promo_rule_condition_customer_conditions_numsales')
                ->setValue(null)
                ->setConditions(array())
                ->setActions(array());
    }
    ...
    public function asHtml() {
        $html = $this->getTypeElement()->getHtml() .
                Mage::helper('giftpromo')->__("If the customer has %s %s completed sales (<b>equals to 0</b> times = no sales)", $this->getOperatorElement()->getHtml(), $this->getValueElement()->getHtml());
        if ($this->getId() != '1') {
