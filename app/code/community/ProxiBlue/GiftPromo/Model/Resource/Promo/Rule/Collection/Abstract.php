 ...
 */
abstract class ProxiBlue_GiftPromo_Model_Resource_Promo_Rule_Collection_Abstract extends Mage_Core_Model_Mysql4_Collection_Abstract { //Mage_Core_Model_Resource_Db_Collection_Abstract {
    ...
    protected function _afterLoad() {
        parent::_afterLoad();
        // populates website real names for grid display.
    ...
    public function addWebsiteFilter($websiteId) {
        if (!$this->getFlag('is_website_table_joined')) {
            ...
            $this->getSelect()->
                    where(' (find_in_set(?, cast(website_ids as char)) > 0) ', $websiteId);
        }
        return $this;
    }
    ...
    public function addCustomerFilter($customerId) {
        if (!$this->getFlag('is_customer_table_joined')) {
            ...
            $this->getSelect()->
                    where(' (find_in_set(?, cast(customer_ids as char)) > 0) ', $customerId);
        }
        return $this;
    }
