...
*/
abstract class TBT_Bss_Model_CatalogSearch_Relevence_Algorithm_TagNameMatch extends TBT_Bss_Model_CatalogSearch_Relevence_Algorithm_Abstract
{
    protected function _assureProductTag( TBT_Bss_Model_CatalogSearch_Mysql4_Fulltext_Collection &$search_query_collection)
    {
        // if option is not enabled in Admin, don't go further
        if (!Mage::helper('bss/config')->isTagMatchEnabled()) {
            return $this;
        }

        if ($search_query_collection->hasJoined('tag_name') || $search_query_collection->hasJoined('cat_name')) {
            $search_query_collection->rememberJoin('tag_name');
            return $this;
        }
