...
class TBT_Testsweet_Model_Test_Suite_Magento_Module_Cache extends TBT_Testsweet_Model_Test_Suite_Abstract {
    ...
    protected function generateSummary() {
        ...
        $known_cacheing_modules = array(/*'Mage_Compiler', 'Mage_PageCache',*/ 'Fooman_Speedster');

        $modules = (array) Mage::getConfig()->getNode('modules')->children();
        
        foreach ($modules as $key => $modele) {
            if (in_array((string)$key, $known_cacheing_modules)) {
                $this->addNotice($this->__('Module: %s - is active and is known to cache data.', $key), $this->__('When caching is too aggressive you might find old data or errors in pages.'));
            }
        }
    }
