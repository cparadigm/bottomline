...
FEED;
        ...
        if (!extension_loaded('suhosin')) {
            ...
            $this->addNotice($this->__("PHP has extension Suhosin."), $this->__("If Magento has issues, try these configuration on top of the default values: %s", $suhosin_help));
        }

        if (strpos($phpinfo, "Suhosin Patch") === false) {
            $this->addPass($this->__("Suhosin is not compiled directly into the PHP binary."));
        } else {
            $this->addNotice($this->__("Suhosin is compiled directly into the PHP binary."), $this->__("If Magento has issues, try these configuration on top of the default values: %s", $suhosin_help));
        }
