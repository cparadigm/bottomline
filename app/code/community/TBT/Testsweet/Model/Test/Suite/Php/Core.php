...
class TBT_Testsweet_Model_Test_Suite_Php_Core extends TBT_Testsweet_Model_Test_Suite_Abstract {
    ...
    protected function generateSummary() {
        ...
        if (ini_get('max_execution_time') == 0) {
            ...
            $this->addWarning($this->__("Max execution time is less than 3600 seconds (1 hour) PHP reports: %s seconds.", ini_get('max_execution_time')), $this->__("Some tasks might require more time to complete and will not finish.  This is often seen as a timeout message."));
        } else {
            $this->addFail($this->__("Max execution time is less than 600 seconds (10 min) PHP reports: %s seconds.", ini_get('max_execution_time')), $this->__("Some tasks can require extra time to complete.  This is often seen as a timeout message in your web browser.  You are advised to set the PHP max_execution_time to 3600."));
        }
    }
