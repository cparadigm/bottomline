    ...
    */
    ...
    protected function _copyImageFile($fromDir, $toDir, &$filename, $import=false, $oldValue=null, $noCopyFlag=false)
    {
        ...
        if(null === $this->_exportImageRetainFolders){
            $this->_exportImageRetainFolders = $this->_profile->getData('options/export/image_retain_folders');
        }
     ...
     * @return string
     */
    protected function catBuildPath($row, $entities, $dataKey = "url_key", $parentKey = "url_key")
    {
        $path = null;
        ...
        if (!empty($row[$dataKey])) {
            ...
            array_pop($ancestorIds); // remove current cat id
            $urlKeys = array();
            foreach ($ancestorIds as $aid) {
                if (!isset($entities[$aid])) {
                    $this->_profile->getLogger()
                    ->warning(sprintf("Parent category with id: %d not found. Category id: %d", $aid, $eId));
                    return $path;
                }
