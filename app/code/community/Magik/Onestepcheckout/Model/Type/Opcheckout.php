<?php
class Magik_Onestepcheckout_Model_Type_Opcheckout
{
    ...
    private function _baseData()
    {
        ...
        if (!$customer || !$addresses)
        {
            ...
            {
            	$country_id	= Mage::getStoreConfig('mgkonestepcheckout_section/mgkopcgeneral_group/mgkopccountry');
    ...
    public function usePayment($method_code = null)
    {
		...
		$method_code = Mage::getStoreConfig('mgkonestepcheckout_section/mgkopcgeneral_group/mgkopcpaymentmethod');
                foreach ($payments as $payment)
                {
                    if ($method_code !== $payment->getCode())
                        continue;
    ...
    public function validateAddress($address)
    {
        ...
        $address->implodeStreetAddress();
        $a_form = Mage::getStoreConfig('mgkonestepcheckout_section/mgkopcfield_group');
