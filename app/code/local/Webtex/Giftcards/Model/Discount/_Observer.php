    ...
    public function testDiscountQuote($observer)
    {
        ...
        if ($session->getActive() && $session->getGiftCardsIds()) {
            ...
            foreach($this->_cards as $id => $_card) {
                $this->_giftCardBalance = $_card['balance'];
                $this->_cardCode = $id;
...
//            return true;
    ...
    {
        $orderItemsModel = Mage::getModel('giftcards/orderitems');
        ...
        foreach ($items as $item) {
            ...
            $orderItem = $orderItemsModel->loadItem($this->_cardCode, $this->_oQuote->getId(), $item->getId()); 
            ...
            if(Mage::helper('tax')->applyTaxAfterDiscount()){
                ...
                $baseItemPrice = $item->getBasePrice($item) * $qty;
            } else {
            ...
            if(Mage::helper('tax')->applyTaxAfterDiscount() && $taxRate){
                $item->setTaxAmount(Mage::app()->getStore()->roundPrice(($itemPrice*100) / $taxRate));
                $item->setBaseTaxAmount(Mage::app()->getStore()->roundPrice(($baseItemPrice*100) / $taxRate));
            }
            ...
            $orderItem->setQuoteId($item->getQuote()->getId());
            $orderItem->setQuoteItemId($item->getId());
            $orderItem->setDiscount($discount/$qty);
            $orderItem->setGiftcardId($this->_cardCode);
    ...
    private function _setBalanceByCurrencies($giftCardsIds)
    {
        ...
        foreach($cards as $card) {
            ...
            $this->_cards[$card->getId()] = array('balance' => $balance - $balanceForFront, 'code' => $card->getCardCode());
