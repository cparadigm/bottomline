...
class AW_Followupemail_Model_Filter extends Mage_Core_Model_Email_Template_Filter
{
    ...
    /**
     * Style definition regular expression
     */
    const STYLE_DEFINITION_PATTERN = '/<style[\w]*[^>]*>(.*?)<\/style>/si';
    ...
    {
        $this->_includeProcessor = array(Mage::getModel('followupemail/rule'), 'getInclude');
        return $this;
    }
    ...
    /**
     * Searches for <style>...</style> definitions and cuts it's content from the source
     * @param string $source
     */
    protected function _cutCss(&$source)
    {
        preg_match_all(self::STYLE_DEFINITION_PATTERN, $source, $definitions, PREG_SET_ORDER);
        foreach ($definitions as $definition) {
            $replacing = '';
            for ($i = 0; $i < strlen($definition[1]); $i++) {
                $replacing .= ' ';
            }
            $source = str_replace($definition[1], $replacing, $source);
        }
    }
    ...
    */
    ...
    public function modifierFormatDateTime($value)
    {
        ...
        if (false === $date || (isset($date['error_count']) && $date['error_count'])) {
            return $value;
        }
