...
class AW_Followupemail_Model_Rule extends Mage_Core_Model_Abstract
{
    ...
    public function getInclude($templateCode, array $variables)
    {
        ...
        $content = $this->_getTemplate($templateCode);
        $template = Mage::getModel('core/email_template')
            ->setData($content)
            ->setTemplateFilter($this->_getProcessor());
    ...
    protected function _getTemplate($templateId, $storeId = null)
    {
        ...
        if (!$pos) {
            ...
            $template['content'] = $template['template_text'];
    ...
    protected function _getContent($objects, $templateId)
    {
        ...
        $template = Mage::getModel('core/email_template')
            ->setData($content)
            ->setTemplateFilter($this->_getProcessor());
        $content['content'] = $template->getProcessedTemplate($objects);
        $content['subject'] = $template->getProcessedTemplateSubject($objects);
    ...
    protected function _createObjects($params, $objects = array())
    {
        ...
        if (Mage::helper('core')->isModuleEnabled('Enterprise_Reward') && isset($objects['customer'])
    ...
    public function process($params, $objects = array())
    {
        ...
        if ($this->_isValid) {
            ...
            foreach ($chain as $chainItem) {
                ...
                $objects['url_unsubscribe_from_all'] = $objects['store']->getUrl(
                    'followupemail/index/unsubscribe', array('code' => $code, 'from_all' => true)
                );
