...
class AW_Followupemail_Model_Events
{
    ...
    */
    ...
    public function newsletterSubscribe($event)
    {
        ...
        if ($subscriber->getData($statusKey) === 1) {
            $queue->cancelByEvent(
                $subscriber->getData($emailKey),
                AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_CUSTOMER_NEW_SUBSCRIPTION
            );
        } else {
            $queue->cancelByEvent(
                $subscriber->getData($emailKey),
    ...
    {
        $confirmation = $customer->getData('confirmation');
        $confirmedEarlier = $customer->getOrigData('confirmation');
        if (
            ($customer->getIsNewCustomer() && !$customer->isConfirmationRequired())
            || (is_null($confirmation) && !is_null($confirmedEarlier))
        ) {
            return true;
        }
        return false;
    ...
    public function checkIsNewCustomer($event)
    {
