...
class AW_Followupemail_Model_Cron
{
    ...
    protected function _removeOldCoupons()
    {
        ...
        $where = $adapter->quoteInto('coupon_id IN (?)', $collection->getAllIds());
        $adapter->delete($collection->getMainTable(), $where);
        ...
        while ($currentPage <= $_pages) {
            ...
            foreach ($statusHistoryCollection->getItems() as $historyItem) {
                ...
                if (
                    ...
                    || $eventName == $ruleTypeOrderStatusPrefix . 'new_order'
                ) {
    ...
    protected function _checkAbandonedCarts()
    {
        ...
        $select = $read->select()
            ->from(
                array('q' => $resource->getTableName('sales/quote')),
                array(
                    'store_id'    => 'q.store_id',
    ...
    protected function _processCustomerActivity($collection, $ruleType)
    {
        ...
        $ruleIds = Mage::getModel('followupemail/mysql4_rule')->getRuleIdsByEventType($ruleType);
    ...
    protected function _checkCustomerReview()
    {
        if (self::DEBUG_MODE) {
            AW_Followupemail_Model_Log::log("Checking customer reviews");
        }
        $resource = Mage::getSingleton('core/resource');
        $read = $resource->getConnection('core_read');
        $select = $read->select()
            ->from(
                array('r' => $resource->getTableName('review')),
                array(
                    'review_id'      => 'r.review_id',
                    'created_at'    => 'r.created_at',
                    'product_id' => 'r.entity_pk_value'
                )
            )
            ->joinInner(
                array('d' => $resource->getTableName('review_detail')),
        ...
        foreach ($reviews as $review) {
            $queue->cancelByEvent(
                ...
                AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_CUSTOMER_REVIEW
            );
        }
        $ruleIds = Mage::getModel('followupemail/mysql4_rule')->getRuleIdsByEventType(
            AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_CUSTOMER_REVIEW
        ...
        $reviewProduct = Mage::getModel('catalog/product');
        foreach ($reviews as $review) {
            $reviewProduct->unsetData()->load($review['product_id']);

            $params = array();
            $params['customer_id'] = $review['customer_id'];
            $params['customer_email'] = $review['customer_email'];
            $params['store_id'] = $review['store_id'];
            $params['category_ids'] = Mage::helper('followupemail')->noEmptyValues(
                array_unique($reviewProduct->getCategoryIds())
            );
            $params['product_ids'] = array($review['product_id']);

            foreach ($ruleIds as $ruleId) {
                $queueCollection = Mage::getModel('followupemail/queue')->getCollection();
                $queueCollection->getSelect()->where('object_id = ?', $review['review_id']);
                $queueCollection->getSelect()->where('rule_id = ?', $ruleId);
                if (sizeof($queueCollection) > 0) {
                    AW_Followupemail_Model_Log::log(
                        'dublicated review Id=' . $review['review_id'] . ' , ruleId=' . $ruleId
                    );
                    continue;
                }
                AW_Followupemail_Model_Log::log(
                    'new review Id=' . $review['review_id'] . ' validating, ruleId=' . $ruleId
                );
                $objects = array();
                $objects['object_id'] = $review['review_id'];
                Mage::getModel('followupemail/rule')->load($ruleId)->process($params, $objects);
            }
        }
    }
